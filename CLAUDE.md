...
- **`py/specex/`** -- the Python/JAX port (the actual subject of this branch).
  - `specex.py` -- CLI entry point (`python -m specex.specex`) and the
    ...
    **Renamed to `run_specex_cpp` and back to `run_specex` twice now**:
    2026-09-13 renamed, reverted same day; 2026-09-11 redone once this
    branch's validation was finished and pushed; 2026-09-13 reverted again
    to let it keep running as normal against `../desispec`'s current,
    unpatched `from specex.specex import run_specex`. The rename is still
    the intended mechanism for when the real desispec integration actually
    happens -- see `docs/python-port/desispec-integration-plan.md` --
    just not while this name is still needed for ordinary use.
    `read_preproc_cpp()`/`read_preproc()`
