...
### Still open / good next-session leads (additions)
- Not yet checked: C++'s warm-start mechanism for trace specifically -- does `Trace::Fit` (per-fiber, from-scratch weighted least squares, `specex_trace.cc:45-90`) get called once as an initial seed before the joint multi-fiber Newton refinement, giving C++ a much-closer-to-truth starting point on bundles far from the input template, while Python's `tc` always starts at exactly zero correction? This is a plausible remaining lever, structurally different from the four ruled out above, not yet tested.
