...
def apply_dead_column_mask(psf, fiber_min, fiber_max, weight):
    w_new = weight.copy(); rows_j = np.arange(weight.shape[1]).astype(float)
    for fib in range(fiber_min, fiber_max + 1):
        try:
            w_v = psf.fiber_traces[fib]['Y_vs_W'].invert(rows_j)
            x_v = psf.x_ccd(fib, w_v); ic = np.floor(x_v + 0.5).astype(int)
            v = (rows_j >= 0) & (rows_j < weight.shape[1]) & (ic >= 0) & (ic < weight.shape[0])
            bad = weight[ic[v], rows_j[v].astype(int)] == 0
            for j in np.where(v)[0][bad]:
                i0, i1 = max(0, ic[j]-4), min(weight.shape[0], ic[j]+5); w_new[i0:i1, j] = 0.0
        except: continue
    return w_new

def get_bundle_footprint(psf, spots, fiber_min, fiber_max, weight=None):
    margin = 0; nx, ny = weight.shape if weight is not None else (4114, 4128)
...
# --- Robust Stable GPU Accumulation ---
...
def accumulate_bundle_gpu_jnp(flux, psf_coeffs, trace_coeffs, continuum, 
    ...
    Ns = flux.shape[0]; Npoly = monomials.shape[1]; Nparams = psf_coeffs.shape[0]
    Np = xpix.shape[0]; Nsh = (Nparams + 2) * Npoly; Ntot = Ns + Nsh + 1; area = stamps_x.shape[1]
    ...
    total_sig = tsig_p[:Np] + continuum; res = image_data - total_sig; chi2 = jnp.sum(weight_data * res**2)
    batch_size = 100; n_batches = (Ns + batch_size - 1) // batch_size; n_padded = n_batches * batch_size
    gh_p, xc_p, yc_p = jnp.pad(gh_all, ((0, n_padded - Ns), (0, 0))), jnp.pad(xc_all, (0, n_padded - Ns)), jnp.pad(yc_all, (0, n_padded - Ns))
    f_p, m_p = jnp.pad(flux, (0, n_padded - Ns)), jnp.pad(monomials, ((0, n_padded - Ns), (0, 0)))
    sx_p, sy_p = jnp.pad(stamps_x, ((0, n_padded - Ns), (0, 0))), jnp.pad(stamps_y, ((0, n_padded - Ns), (0, 0)))
    idx_p, mask_p = jnp.pad(stamp_indices, ((0, n_padded - Ns), (0, 0)), constant_values=Np), jnp.arange(n_padded) < Ns
    def scan_body(carry, b_step):
        ...
        def spot_jac(x, y, g, fi, mi, s_x, s_y):
            ...
            return p_s, jnp.concatenate([(fi * jg[:, :, jnp.newaxis] * mi).reshape(area, -1), (fi * jx[:, jnp.newaxis] * mi), (fi * jy[:, jnp.newaxis] * mi)], axis=1)
    ...
    B_f = B_f.at[-1].set(jnp.sum(weight_data * res)); A_f = A_f.at[-1, -1].set(jnp.sum(weight_data))
    return chi2, A_f, B_f
...
class PSF_Fitter:
    ...
    def fit(self, image, weight, spots, bundle_id, fit_type='full', max_iter=50):
        ...
        tc = jnp.zeros((2, monomials.shape[1])); cc = 0.0; img_d, w_d = jnp.array(image[xpix, ypix]), jnp.array(weight[xpix, ypix]); old_chi2 = 1e30
        for i in range(max_iter):
            chi2, A, B = accumulate_bundle_gpu_jnp(flux, pc, tc, cc, xc_init, yc_init, monomials, img_d, w_d, jnp.array(xpix), jnp.array(ypix), self.psf.gh_psf.degree, jnp.array(sx), jnp.array(sy), jnp.array(idx_g))
            mode = 'flux' if i < 2 else 'trace' if i < 7 else 'full'
            print(f"Iter {i}: chi2 = {float(chi2):.4f} [Mode: {mode}]")
            Ns_l, Np_l = len(flux), monomials.shape[1]
            if mode == 'flux': idx = jnp.concatenate([jnp.arange(Ns_l), jnp.array([A.shape[0]-1])])
            elif mode == 'trace': idx = jnp.concatenate([jnp.arange(Ns_l), jnp.arange(Ns_l+55*Np_l, Ns_l+57*Np_l), jnp.array([A.shape[0]-1])])
            ...
            flux = jnp.maximum(flux + alpha * d_p[:Ns_l], 0.0); pc = pc + alpha * d_p[Ns_l : Ns_l+55*Np_l].reshape(55, Np_l); tc = tc + alpha * d_p[Ns_l+55*Np_l : Ns_l+57*Np_l].reshape(2, Np_l); cc = cc + alpha * d_p[-1]
            if mode == 'full' and jnp.abs(old_chi2 - chi2) < self.chi2_precision: break
            old_chi2 = chi2
        return float(old_chi2), np.array(pc), np.array(tc), float(cc)
