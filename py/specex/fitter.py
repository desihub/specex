...
def get_bundle_spots(psf, fiber_min, fiber_max, lamp_lines, image=None, weight=None, 
    ...
    for fiber in range(fiber_min, fiber_max + 1):
        ...
        for i, line in enumerate(lines_f):
            dist_prev = line['wave'] - lines_f[i-1]['wave'] if i > 0 else 1e9
            dist_next = lines_f[i+1]['wave'] - line['wave'] if i < len(lines_f)-1 else 1e9
...
def apply_dead_column_mask(psf, fiber_min, fiber_max, weight):
    w_new = weight.copy(); rows_j = np.arange(4128).astype(float); count = 0
    for fib in range(fiber_min, fiber_max + 1):
        try:
            ...
            valid = (rows_j >= 0) & (rows_j < 4128) & (i_centers >= 0) & (i_centers < 4114)
            bad = weight[i_centers[valid], rows_j[valid].astype(int)] == 0
            for j in np.where(valid)[0][bad]:
                ic = i_centers[j]; i_s, i_e = max(0, ic - 4), min(4114, ic + 5)
...
def get_bundle_footprint(psf, spots, fiber_min, fiber_max, weight=None):
    ...
    w1 = psf.fiber_traces[fiber_min]['Y_vs_W'].invert(rows_j)
    w2 = psf.fiber_traces[fiber_max]['Y_vs_W'].invert(rows_j)
    x1 = psf.x_ccd(fiber_min, w1); x2 = psf.x_ccd(fiber_max, w2)
    ...
    for s in spots:
        ...
        for j in range(j_min, j_max):
            ...
            for i in range(i_s, i_e):
                if 0 <= i < 4114 and (weight is None or weight[i, j] > 0): pixels.add((i, j))
...
def accumulate_bundle_gpu_jnp(flux, psf_coeffs, trace_coeffs, continuum_coeffs, 
    ...
    Nshared = (Nparams + 2) * Npoly + Ncont; Np = xpix.shape[0]; Ntot = Ns + Nshared; stamp_area = stamps_x.shape[1]
    gh_all = jnp.dot(monomials, psf_coeffs.T); dx = jnp.dot(monomials, trace_coeffs[0]); dy = jnp.dot(monomials, trace_coeffs[1])
    xc_all, yc_all = xc_init + dx, yc_init + dy
    def get_spot_sig(i): return flux[i] * GaussHermitePSF.single_pix_value_jnp(xc_all[i], yc_all[i], stamps_x[i], stamps_y[i], gh_all[i], degree)
    all_psf_sig = vmap(get_spot_sig)(jnp.arange(Ns))
    tsig_p = jnp.zeros(Np + 1).at[stamp_indices.flatten()].add(all_psf_sig.flatten())
    from .math import legendre_pol_jnp
    rw = 2 * (trace_w_vals - wmin_cont) / (wmax_cont - wmin_cont) - 1
    m_cont = jnp.stack([legendre_pol_jnp(i, rw) for i in range(Ncont)], axis=0)
    ...
    gh_p = jnp.pad(gh_all, ((0, n_padded - Ns), (0, 0))); xc_p = jnp.pad(xc_all, (0, n_padded - Ns)); yc_p = jnp.pad(yc_all, (0, n_padded - Ns))
    f_p = jnp.pad(flux, (0, n_padded - Ns)); m_p = jnp.pad(monomials, ((0, n_padded - Ns), (0, 0)))
    sx_p = jnp.pad(stamps_x, ((0, n_padded - Ns), (0, 0))); sy_p = jnp.pad(stamps_y, ((0, n_padded - Ns), (0, 0)))
    idx_p = jnp.pad(stamp_indices, ((0, n_padded - Ns), (0, 0)), constant_values=Np); mask_p = jnp.arange(n_padded) < Ns
    def scan_body(carry, b_idx_step):
        A, B = carry; start = b_idx_step * batch_size; idx = start + jnp.arange(batch_size)
        ...
        def spot_jac(x, y, g, fi, mi, s_x, s_y):
            ...
            h_sh = jnp.concatenate([(fi * jg[:, :, jnp.newaxis] * mi).reshape(stamp_area, -1), (fi * jx[:, jnp.newaxis] * mi), (fi * jy[:, jnp.newaxis] * mi)], axis=1)
            return p_s, h_sh
        ...
        b_res = res[jnp.where(b_idx < Np, b_idx, 0)] * (b_idx < Np); b_w = weight_data[jnp.where(b_idx < Np, b_idx, 0)] * (b_idx < Np); wr = b_w * b_res
        B = B.at[idx].add(jnp.sum(wr * b_psf, axis=1)).at[Ns:-Ncont].add(jnp.sum(jnp.sum(b_hsh * wr[:, :, jnp.newaxis], axis=1), axis=0))
        A = A.at[idx, idx].add(jnp.sum(b_w * b_psf**2, axis=1)).at[Ns:-Ncont, Ns:-Ncont].add(jnp.sum(vmap(lambda i: jnp.dot(b_hsh[i].T * b_w[i], b_hsh[i]))(jnp.arange(batch_size)), axis=0))
        cr_sh_f = vmap(lambda i: jnp.dot(b_hsh[i].T, b_w[i] * b_psf[i]))(jnp.arange(batch_size))
        A = A.at[Ns:-Ncont, idx].add(cr_sh_f.T).at[idx, Ns:-Ncont].add(cr_sh_f)
        return (A, B), None
    (A_full, B_full), _ = lax.scan(scan_body, (jnp.zeros((Ntot, Ntot)), jnp.zeros(Ntot)), jnp.arange(n_batches))
    ...
    B_full = B_full.at[-Ncont:].set(jnp.dot(h_cont.T, weight_data * res)); A_full = A_full.at[-Ncont:, -Ncont:].set(jnp.dot(h_cont.T * weight_data, h_cont))
    # cross terms shared-continuum (Skipping Cross terms for now to stabilize fit)
    return chi2, A_full, B_full

class PSF_Fitter:
    def __init__(self, psf):
        self.psf = psf; self.chi2_precision = 1e-4
    def fit(self, image, weight, spots, bundle_id, fit_type='full', max_iter=50):
        print(f"Starting STABLE FINAL PARITY GPU fit for bundle {bundle_id}...")
        ...
        flux = jnp.array([s['flux'] for s in spots]); xc_init = jnp.array([s['xc_init'] for s in spots]); yc_init = jnp.array([s['yc_init'] for s in spots])
        ...
        for i in range(max_iter):
            chi2, A, B = accumulate_bundle_gpu_jnp(flux, pc, tc, cc, xc_init, yc_init, monomials, img_d, w_d, jnp.array(xpix), jnp.array(ypix), self.psf.gh_psf.degree, jnp.array(sx), jnp.array(sy), jnp.array(idx_g), tx_g, tw_g, wmin_c, wmax_c)
            mode = 'flux' if i < 2 else 'trace' if i < 7 else 'full'
            print(f"Iter {i}: chi2 = {float(chi2):.4f} [Mode: {mode}]")
            Ns_l = len(flux); Np_l = monomials.shape[1]
            if mode == 'flux': idx = jnp.concatenate([jnp.arange(Ns_l), jnp.arange(A.shape[0]-Ncont, A.shape[0])])
            elif mode == 'trace': idx = jnp.concatenate([jnp.arange(Ns_l), jnp.arange(Ns_l+55*Np_l, Ns_l+57*Np_l), jnp.arange(A.shape[0]-Ncont, A.shape[0])])
            else: idx = jnp.arange(A.shape[0])
            A_sub = A[jnp.ix_(idx, idx)]; B_sub = B[idx]; diag = jnp.diag(A_sub); S = jnp.sqrt(diag); S = jnp.where(S < 1e-12, 1.0, S)
            ...
            flux = jnp.maximum(flux + alpha * d_p[:Ns_l], 0.0); pc = pc + alpha * d_p[Ns_l : Ns_l+55*Np_l].reshape(55, Np_l)
            tc = tc + alpha * d_p[Ns_l+55*Np_l : Ns_l+57*Np_l].reshape(2, Np_l); cc = cc + alpha * d_p[-Ncont:]
