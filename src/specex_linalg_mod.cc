...
double specex::dot(const unhrp::vector_double& v1, const unhrp::vector_double& v2) {
#ifdef CHECK_BOUNDS
  if(v1.size() != v2.size())
...
void specex::syr(const double& w, const unhrp::vector_double& h, unhrp::matrix_double& A) {
...
void specex::axpy(const double &a, const unhrp::vector_double& h,  unhrp::vector_double& B) {
  blas::axpy(a,h,B);
}
// ! B := alpha*A*h + beta*B (B += alpha*A*h for beta=1)
void specex::gemv(const double &alpha,  const unhrp::matrix_double &A,  const unhrp::vector_double& h, const double &beta, unhrp::vector_double& B) {
...
void specex::symm(const char side, const double& alpha, const unhrp::matrix_double &A, const unhrp::matrix_double &B, const double& beta, unhrp::matrix_double &C) {
