/*
  High Performance Astrophysical Reconstruction and Processing (UNHRP)
  (c) 2014-2015, The Regents of the University of California, 
  through Lawrence Berkeley National Laboratory.  See top
  level LICENSE file for details.
*/

#include <unhrp_data_internal.hpp>
...
using namespace std;
using namespace unhrp;


void unhrp::fits::check ( int status ) {
  if ( status ) {
    ...
    UNHRP_THROW( o.str().c_str() );
  }
  return;
}


void unhrp::fits::close ( fitsfile * fp ) {
...
void unhrp::fits::open_read ( fitsfile * & fp, string const & path ) {
...
void unhrp::fits::open_readwrite ( fitsfile * & fp, string const & path ) {
...
void unhrp::fits::create ( fitsfile * & fp, string const & path ) {
...
int unhrp::fits::nhdus ( fitsfile * fp ) {
  int nhdu;
...
void unhrp::fits::key_strclean ( std::string & val ) {
...
bool unhrp::fits::key_exclude ( std::string const & name ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, std::string & val ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, bool & val ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, double & val ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, float & val ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, long long int & val ) {
...
void unhrp::fits::key_read ( fitsfile * fp, std::string const & keyname, int & val ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, std::string const & keyval, std::string const & keycom ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, char * keyval, std::string const & keycom ) {
  string keystr = keyval;
  key_write ( fp, keyname, keystr, keycom );
  return;
}

void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, char const * keyval, std::string const & keycom ) {
  string keystr = keyval;
  key_write ( fp, keyname, keystr, keycom );
  return;
}
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, bool const & keyval, std::string const & keycom ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, double const & keyval, std::string const & keycom ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, float const & keyval, std::string const & keycom ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, int const & keyval, std::string const & keycom ) {
...
void unhrp::fits::key_write ( fitsfile * fp, std::string const & keyname, long long int const & keyval, std::string const & keycom ) {
...
boost::property_tree::ptree unhrp::fits::key_read_all ( fitsfile * fp ) {
...
void unhrp::fits::key_write_all ( fitsfile * fp, boost::property_tree::ptree const & keys ) {
  ...
  while ( v != keys.end() ) {
    ...
    if ( child.get < string > ("TYPE") == "C" ) {
      ...
      UNHRP_THROW( "unknown FITS key type" );
    }
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, std::string & val ) {
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, bool & val ) {
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, long long int & val ) {
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, int & val ) {
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, double & val ) {
...
void unhrp::fits::key_parse ( boost::property_tree::ptree const & keys, std::string const & name, float & val ) {
...
void unhrp::fits::key_require ( boost::property_tree::ptree const & keys, std::string const & name, std::string const & type ) {
...
int unhrp::fits::seek ( fitsfile * fp, string const & extname ) {
  int hdu;
...
int unhrp::fits::img_seek ( fitsfile * fp, std::string const & keyname, std::string const & keyval ) {
  int hdu;
...
void unhrp::fits::img_seek ( fitsfile * fp, int hdu ) {
...
int unhrp::fits::bin_seek ( fitsfile * fp, string const & keyname, string const & keyval ) {
  int hdu;
...
void unhrp::fits::bin_seek ( fitsfile * fp, int hdu ) {
...
void unhrp::fits::img_dims ( fitsfile * fp, size_t & rows, size_t & cols ) {
...
void unhrp::fits::bin_create ( fitsfile * fp, std::string extname, size_t nrows, std::vector < std::string > colnames, std::vector < std::string > coltypes, std::vector < std::string > colunits ) {

  size_t ncols = colnames.size();

  if ( ( ncols != coltypes.size() ) || ( ncols != colunits.size() ) ) {
    UNHRP_THROW( "vectors of column names, types, and units do not have equal lengths" );
  }
...
void unhrp::fits::bin_info ( fitsfile * fp, size_t & nrows, vector < string > & colnames ) {
  ...
  for ( int i = 0; i < tfields; ++i ) {
    ...
    if ( colnum != i + 1 ) {
      UNHRP_THROW( "error in binary table column name lookup" );
    }
    colnames[i] = colnam;
  }
  status = 0;
...
vector < int > unhrp::fits::bin_columns ( fitsfile * fp, vector < string > & names ) {
...
void unhrp::fits::bin_read_column_strings ( fitsfile * fp, size_t firstrow, size_t lastrow, int col, vector < string > & data ) {
  ...
  if ( offset + nread > nrows ) {
    UNHRP_THROW( "binary read range is beyond end of table" );
  } 
  ...
  if ( ! charray ) {
    UNHRP_THROW( "cannot allocate temp char array" );
  }
  for ( long i = 0; i < nread; ++i ) {
    charray[i] = (char*) malloc ( FLEN_VALUE * sizeof (char) );
    if ( ! charray[i] ) {
      UNHRP_THROW( "cannot allocate temp char array member" );
    }
  }
...
void unhrp::fits::bin_write_column_strings ( fitsfile * fp, size_t firstrow, size_t lastrow, int col, vector < string > & data ) {
  ...
  if ( offset + nwrite > nrows ) {
    UNHRP_THROW( "binary write range is beyond end of table" );
  } 
...
void unhrp::fits::test ( string const & datadir ) {
