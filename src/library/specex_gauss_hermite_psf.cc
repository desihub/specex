...
void specex::GaussHermitePSF::WriteFits_v1(fitsfile* fp, int first_hdu) const {
  
  // this is a nasty temporary hack
  const_cast< specex::GaussHermitePSF*>(this)->SetDegree(degree);
  ...
  for(std::map<int,PSF_Params>::const_iterator bundle_it = ParamsOfBundles.begin();
    ...
    if(bundle_it == ParamsOfBundles.begin()) {
  ...
  SPECEX_INFO("GaussHermitePSF::WriteFits(1) number of parameters = " << nparams);
  SPECEX_INFO("GaussHermitePSF::WriteFits(1) max number of Legendre coefficients = " << ncoeff);
  ...
  { // column description
    ...
    sprintf(coeff_tdim,"( %d, %d)",ncoeff,NFIBERS);
    table.AddColumnDescription("COEFF",coeff_tform,coeff_tdim,"");
    //SPECEX_INFO("GaussHermitePSF::WriteFits(1) columns ");
  ...
  */
  ...
  { // data
    image = specex::image_data(ncoeff*(nparams+1),NFIBERS);
    image.data.clear();
    ...
    SPECEX_INFO("GaussHermitePSF::WriteFits(1) LEGWMIN  " << LEGWMIN);
    SPECEX_INFO("GaussHermitePSF::WriteFits(1) LEGWMAX  " << LEGWMAX);
	...
	SPECEX_INFO("GaussHermitePSF::WriteFits(1) adding param " << "GH-0-0");
	keys.push_back("GH-0-0"); for(int f=0;f<NFIBERS;f++) image(0,f)=1;	
	need_to_add_first_gh = false;
      }

      SPECEX_INFO("GaussHermitePSF::WriteFits(1) adding param " << pname);
	...
	const Legendre2DPol_p pol2d = params_of_bundle.AllParPolXW[p]; // this is the 2D polynomiald of x_ccd and wave for this param and bundle
	

	
	for(int fiber=params_of_bundle.fiber_min; fiber<=params_of_bundle.fiber_max; fiber++,fiber_index++) {
	  
	  const specex::Trace& trace = FiberTraces.find(fiber)->second; // X_vs_W.Value();

	  // build a Legendre1DPol out of the Legendre2DPol
	  Legendre1DPol pol1d(ncoeff-1,wavemin,wavemax);
    ...
    SPECEX_WARNING("GaussHermitePSF::WriteFits(1) missing psf tail properties");
