...
bool specex::PSF_Fitter::FitSeveralSpots(vector<specex::Spot_p>& spots, double *chi2, int *n_pixels, int *n_iterations) {
  ...
  *niter=0;
  ...
  while(true) { // minimization loop 
    ...
    clock_t tstop = clock();
    if(verbose) SPECEX_INFO("specex::PSF_Fitter::FitSeveralSpots time = " << float(tstop-tstart)/float(CLOCKS_PER_SEC) << "s");
    ...
    */
	...
	if(dchi2>0 || fabs(dchi2)<brent_precision) {
      ...
      *psfChi2 = best_chi2;
      ...
      if(verbose) SPECEX_INFO("specex::PSF_Fitter::FitSeveralSpots dchi2=" << oldChi2-*psfChi2 << " chi2pdf = " << *psfChi2/(*npix-Params.size()));
...
bool specex::PSF_Fitter::FitEverything(std::vector<specex::Spot_p>& input_spots, bool init_psf) {
  ...
  int npix = 0;
  int niter = 0;
  double min_snr_non_linear_terms = 50;
  double min_snr_linear_terms = 1;
  ...
  */
  ...
  // because this can bias strongly sigmas and traces
  for(size_t s=0;s<tmp_selected_spots.size();s++) {
    Spot_p spot= tmp_selected_spots[s];
    specex::Stamp st = Stamp(image);
    SetStampLimitsFromPSF(st,psf,spot->xc,spot->yc);
    bool haszeroweight=false;
    for(int j=st.begin_j;j<st.end_j;j++)
      for(int i=st.begin_i;i<st.end_i;i++)
	if(weight(i,j)==0) {haszeroweight=true; break;}
    if(!haszeroweight) 
      selected_spots.push_back(spot);
  }
  */
  ...
  */
  ...
  selected_spots = select_spots(input_spots,min_snr_linear_terms);
  ...
  if(psf_params->FitParPolXW.size()) {
    ...
    if(0) {
    SPECEX_INFO("Starting FitSeveralSpots PSF (hermite terms)");
    SPECEX_INFO("=======================================");
    fit_flux       = false;
    fit_position   = false;
    fit_psf        = true;
    ...
    SPECEX_INFO("Starting FitSeveralSpots PSF+FLUX (hermite terms)");
    SPECEX_INFO("=======================================");
    fit_flux       = true;
    fit_position   = false;
    fit_psf        = true;
  ...
  fatal=true;
  if(!ok) {
    SPECEX_INFO("Starting FitSeveralSpots PSF+FLUX (bis bis bis bis)");
    SPECEX_INFO("=======================================");
