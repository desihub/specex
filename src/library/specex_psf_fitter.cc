...
double specex::PSF_Fitter::ComputeChi2AB(bool compute_ab) 
{
  ...
  for (int j=stamp.begin_j; j <stamp.end_j; ++j) {
	...
	double tail_val = psf->TailValue(i-tmp.x,j-tmp.y); //tail_amplitude/rs2p1;
	res -= tmp.flux*tail_val;
	if (compute_ab) {
...
// selecting spots
  ...
  fit_psf        = true;
  fit_trace      = false;
  if(!scheduled_fit_of_traces) chi2_precision = 0.1;
