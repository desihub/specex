...
## 2026-09-15 -- Full 30-camera desispec run_gpu() validation: found and fixed a real MPI-rank cpuset-confinement timing gap (1792.5s -> 837.1s); correctness confirmed against real C++ for both Python paths.
...
**Verified**: reran the identical 30-camera real 4-rank MPI test with the fix in place -- `total_wall=837.1s`, `cameras_ok=30/30`, `err=0`, clean barrier on all 4 ranks, no crash. **A 53% reduction** (1792.5s -> 837.1s), though still ~1.7x `run_night.py`'s own 498.7s, not full parity. Leading explanation for the residual gap, not yet directly tested: the other 3 MPI ranks are genuinely idle but still alive, spinning on their own `comm.Barrier()` -- Cray MPICH's default wait mode busy-polls rather than blocking, and since the four ranks' ORIGINAL affinity slices overlap (not disjoint), rank 0's now-widened 128-core pool inevitably shares physical cores with 3 actively-polling (if idle) neighbor processes for the whole run, unlike `run_night.py`'s standalone case where no other process is competing for those same cores at all. Worth a follow-up (e.g. explicitly `MPI.Info` a blocking/low-power wait mode for the idle ranks, or confirm via `mpstat`/`perf` during a rerun) but not chased further this session -- the crash/correctness risk this session was scoped to fix is resolved, and a 53% timing win with zero risk to correctness is a clear net improvement over the unfixed sequential-loop-era `run_gpu()` (which itself was ~20-45min, i.e. this is now roughly 2.5-3x faster again on top of the earlier `fit_cameras_persistent()` refactor).
