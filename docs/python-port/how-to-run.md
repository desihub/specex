...
**`--worker-mode {subprocess, persistent}`** (default: `persistent`, since 2026-09-13) controls how `--backend python` executes across cameras:
*   **`persistent`** (default): one long-lived worker process per GPU, pulling cameras off a shared queue and calling `fit_ccd_native()` in-process instead of paying interpreter/JAX-import startup cost per camera. Also reuses the per-bundle `multiprocessing.Pool` across a GPU worker's whole stream of cameras (`--no-pool-reuse` to disable, A/B-testing only, no correctness effect). **The fastest validated configuration** (see the campaign numbers below) -- promoted to the default once its own real cold-JAX-cache GPU-OOM bug was found and fixed (`porting-notes.md`, 2026-09-10) and it had been exercised across multiple full-night campaigns and two environments (`specex_env` and the shared DESI+JAX module).
*   **`subprocess`**: a fresh `python -m specex.specex` process per camera -- the original methodology every prior campaign's numbers were measured under, kept as an option for its simpler, longer-tenure crash-isolation model (a brand-new process per camera) rather than for any known correctness or speed advantage -- it's slower than C++ (~674s/night).
...
**Other flags:** `--cameras` (restrict to a subset, default all 30), `--nodes` (default: full SLURM allocation), `--gpus-per-node` (default: auto-detect), `--worker-mode {subprocess,persistent}` (default `persistent`, see above), `--no-pool-reuse` (persistent mode only, A/B-testing), `--footprint-margin` (passthrough to `specex.specex --footprint-margin`, default 7), `--dry-run` (print planned commands without executing).

**The multi-node/LPT numbers below predate `--worker-mode persistent` and were measured under `subprocess` mode only** -- they haven't been separately re-validated with `persistent` mode's per-camera-startup savings stacked on top. Since `persistent` is now the default, **explicitly pass `--worker-mode subprocess` for any multi-node + LPT run** until that combination is tested; don't assume the default carries over cleanly to a scope these numbers don't cover. For single-node runs, `persistent` mode (table above, and the default) is faster than any of the multi-node `subprocess` numbers below and doesn't need multiple nodes at all.
...
| 2.3 Full night, 30 cameras, 1 node/4 GPUs, `--worker-mode persistent` (default) | `run_night.py --backend python` | ~483-503s (8-8.4 min) |
| 2.3 Full night, 30 cameras, 1 node/4 GPUs, `--worker-mode subprocess` | `run_night.py --backend python --worker-mode subprocess` | ~11-13 min |
